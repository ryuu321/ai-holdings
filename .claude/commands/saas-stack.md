...
### Streamlit アプリ内 法的 expander テンプレート（app.py 末尾にそのまま貼る）
...
with st.expander("利用規約"):
    st.markdown("""
**第1条（適用）** 本規約は {ProductName} の利用条件を定めます。ご利用をもって同意とみなします。

**第2条（サービスの内容）** 本サービスは{機能説明}のAIツールです。利用前に利用者自身が内容を確認・編集してください。

**第3条（禁止事項）** 虚偽情報の入力・不正アクセス・第三者への再販・著作権侵害となる利用等を禁止します。

**第4条（免責事項）** 生成文章の正確性・完全性を保証しません。生成結果の使用による損害について運営者は責任を負いません。関連法令の適合は利用者の責任で確認してください。
...
st.caption("© 2026 {ProductName}　📧 ryuumg03@gmail.com")
```
