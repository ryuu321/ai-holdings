...
def save_trade(record) -> int:
    ...
    is_short    = int(bool(getattr(record, "is_short", False)))
    # TradeRecord は ticker/shares、互換ラッパーは coin/amount を使う
    coin   = getattr(record, "coin",   None) or getattr(record, "ticker", "")
    amount = getattr(record, "amount", None)
    if amount is None:
        amount = getattr(record, "shares", 0)
    conn = sqlite3.connect(DB_PATH)
    cur = conn.cursor()
    cur.execute("""
        INSERT INTO trades
        (timestamp, action, coin, price, amount, value_usd, balance_after, pnl,
         reasoning, confidence, risk_level, bot_type, signals_json, is_short)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    """, (
        record.timestamp, record.action, coin,
        record.price, amount, record.value_usd,
        record.balance_after, record.pnl, record.reasoning,
        record.confidence, record.risk_level, bot_type, signals_str, is_short,
    ))
    conn.commit()
