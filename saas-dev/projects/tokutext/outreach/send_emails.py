"""
TokuText アウトリーチ送信スクリプト（Gmail SMTP・30件/日）
"""
import argparse
import csv
import os
...
_COMPANY_REQUIRED = ["特別養護", "老人ホーム", "老健", "特養", "介護老人", "ケアホーム", "福祉"]
_BLOG_SIGNALS = ["コツ", "方法", "選び方", "ランキング", "名簿", "営業リスト", "比較", "一覧"]
...
def _check_safety(drafts: list[dict]) -> bool:
    ...
    for d in drafts:
        ...
        if any(sig in name for sig in _BLOG_SIGNALS):
            ...
            print(f"WARNING: {d['email']} — 介護施設名なし: 「{name[:30]}」")
            ok = False
        body = d.get("body", "")
        missing = []
