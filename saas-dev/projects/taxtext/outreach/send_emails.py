"""
TaxText アウトリーチ送信スクリプト（Gmail SMTP・30件/日）
"""
import argparse
import csv
import os
...
_COMPANY_REQUIRED = ["税理士", "会計", "法人", "事務所"]
...
def main(limit: int = DAILY_LIMIT, dry_run: bool = False, test_to: str = "", force_send: bool = False):
    ...
    _extra_blog = ["するなら", "なら", "お任せ", "はこちら", "にお問い合わせ", "について", "するには", "講座"]
    valid_targets = []
    for d in targets:
        name = d.get("company_name", "")
        if any(s in name for s in _BLOG_SIGNALS) or any(s in name for s in _extra_blog):
